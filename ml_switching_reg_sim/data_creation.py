...
def extract_estimator_inputs(df, mw, regimes, lags=None):
    ...
    driver_ids : (N,)  integer driver index per observation
    """
    N = len(df)
    ...
    driver_ids = df.index.get_level_values("driver").values
...
class MisclassificationCreator:
    ...
    def noisify_matrix(self, extent, index):
        """Return a length-R probability vector for a driver whose true regime is *index*.

        For R=2 the off-diagonal mass is distributed symmetrically.  For R>2
        the off-diagonal mass is split randomly via a recursive uniform draw
        (``_recursive_fill_in``), so the resulting matrix is **asymmetric** in
        the off-diagonal entries by construction.  This is intentional: the
        total off-diagonal mass per row equals ``extent * (1 - 1/R)``, but its
        distribution among the R-1 wrong classes is stochastic.
        """
        ...
        def _recursive_fill_in(ext):
            ...
            return _recursive_fill_in(np.append(ext, self.random.uniform(0, ext_diff)))
...
class UberDatasetCreatorHet:
    ...
    def __init__(self, drivers=275, regimes=4, time_periods=10, seed=None, lags=None):
        ...
        self.dates = pd.date_range(
            start="2016-01-01", periods=self.time_periods, freq="ME"
        )
    ...
    def _create_drought_index(self, mean=None, cov=None, mock=False, mock_dict=None):
        ...
        if mock:
            ...
            alpha_r = dict(
                zip(range(self.regimes), self.random.normal(0, 1, size=self.regimes))
            )
            df = pd.DataFrame(
            ...
            for shock, base, amplitude, offset, r, ar in zip(
                ...
                trend = LinearTrend(
                    coefficient=0, time_unit=timedelta(days=30), flat_base=base
                )
                seasonality = SinusoidalSeasonality(
                    amplitude=amplitude, period=timedelta(days=90), offset=offset
                )
                noise_ts = RedNoise(mean=0, std=1, correlation=0.5)
                trans = LinearTransition(
                    transition_window=timedelta(days=30), stop_window=timedelta(days=5)
                )
                switch = Switch(
    ...
    def _create_drought_index_with_driver(
        self, mean=None, cov=None, mock=False, mock_dict=None
    ):
        """Broadcast the time-level drought DataFrame to all drivers."""
        drought_df = self._create_drought_index(
            mean=mean, cov=cov, mock=mock, mock_dict=mock_dict
        )
        drought_cols = [f"drought_{i}" for i in range(self.regimes)]

        driver_time_df = pd.concat(
            [drought_df.assign(driver=i) for i in range(self.drivers)]
        ).set_index("driver", append=True)
        driver_time_df.index.names = ["time", "driver"]

        return (
            driver_time_df.reorder_levels(["driver", "time"])
            .reset_index()
            .pipe(
                lagged_drought_df,
                drought_cols,
                shift=self.lags,
                groupby_index="driver",
                date_col="time",
    ...
    def _build_regime_dummies_noisify(self, regime, weight):
        ...
        """
        m = MisclassificationCreator(
            self.regimes,
            seed=int(self.random.integers(0, np.iinfo(np.uint32).max)),
        )
        ...
        rd = (
            pd.concat(
                [
                    ...
                    regime_with_mc.apply(
                        lambda x: x["misclass_regime"], result_type="expand", axis=1
                    ),
                ],
                axis=1,
            )
            ...
            .assign(
                max_misclass_regime=lambda df: df["misclass_regime"].apply(
                    lambda x: x.argmax()
                )
            )
    ...
    def construct(
        ...
        regime = pd.concat(
            [self._create_driver_index(), self._create_regime_index()], axis=1
        )
        ...
        if classifier_mode == "noisify":
            ...
            df = (
                drought.join(regime_dummies.set_index("driver"))
            ...
            partial_df = drought.join(regime.set_index("driver")).pipe(
                self._create_y, beta0=beta0, beta1=beta1, name=y_name, sd=y_sd
            )
            for r in range(self.regimes):
                partial_df.loc[lambda df: df["regime"] == r, y_name] = partial_df[
                    f"{y_name}_{r}"
                ]
            ...
            clf_seed = (
                seed
                if seed is not None
                else (self.seed if self.seed is not None else 42)
            )
            ...
            df = partial_df.join(regime_dummies.set_index("driver")).pipe(
                pd.get_dummies, columns=["max_misclass_regime"]
            )
        else:
            raise ValueError(
        ...
        alpha_i = (
            {
                k: v
                for k, v in zip(
                    driver_ids, self.random.normal(0, 1, size=len(driver_ids))
                )
            }
            if driver_fe
            else {k: 0 for k in driver_ids}
        )
        alpha_t = (
            {
                k: v
                for k, v in zip(time_ids, self.random.normal(0, 1, size=len(time_ids)))
            }
            if time_fe
            else {k: 0 for k in time_ids}
        )
        alpha_m = (
            {
                k: v
                for k, v in zip(
                    month_ids, self.random.normal(0, 1, size=len(month_ids))
                )
            }
            if month_year_fe
            else {k: 0 for k in month_ids}
        )
        alpha_y = (
            {
                k: v
                for k, v in zip(year_ids, self.random.normal(0, 1, size=len(year_ids)))
            }
            if month_year_fe
            else {k: 0 for k in year_ids}
        )

        df = df.assign(
            alpha_i=df.index.get_level_values("driver").map(alpha_i),
            alpha_t=df.index.get_level_values("time").map(alpha_t),
            alpha_m=pd.DatetimeIndex(df.index.get_level_values("time")).month.map(
                alpha_m
            ),
            alpha_y=pd.DatetimeIndex(df.index.get_level_values("time")).year.map(
                alpha_y
            ),
            y=lambda df: (
                df["y"] + df["alpha_i"] + df["alpha_t"] + df["alpha_m"] + df["alpha_y"]
            ),
        )
